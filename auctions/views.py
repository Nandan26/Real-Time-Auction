...
def index(request):
    ...
    expensive_auctions = Auction.objects.order_by('-starting_bid')[:5]

    for auction in auctions:
        auction.image = auction.get_images.first()

    # top_categories = Auction.objects.all().values('category').annotate(total=Count('category')).order_by('-total')[:7]
...
#     '''
#     auction = Auction.objects.get(id=auction_id)
#     amount = request.POST.get('amount', auction.current_bid)
...
def my_items(request, user_id):
    ...
    winning_items = Auction.objects.filter(buyer_id = user_id)

    for auction in winning_items:
        auction.image = auction.get_images.first()

    # Show 3 active auctions per page
    page = request.GET.get('page', 1)
    paginator = Paginator(winning_items, 3)
    try:
        pages = paginator.page(page)
    except PageNotAnInteger:
        pages = paginator.page(1)
    ...
    return render(request, 'auctions_active.html', {
        'categories': Category.objects.all(),
        'auctions': winning_items,
        'auctions_count': winning_items.count(),
        'pages': pages,
        'title': 'My Items'
    })
